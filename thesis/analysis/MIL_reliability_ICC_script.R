...
# Add reward condition column and rename 1, 2, and 3 to neutral, loss, and gain.
d1$rewardcond = c()
d1[d1$trialtype == 1, 'rewardcond'] = 'neutral'
d1[d1$trialtype == 2, 'rewardcond'] = 'loss'
d1[d1$trialtype == 3, 'rewardcond'] = 'gain'

d2$rewardcond = c()
d2[d2$trialtype == 1, 'rewardcond'] = 'neutral'
d2[d2$trialtype == 2, 'rewardcond'] = 'loss'
d2[d2$trialtype == 3, 'rewardcond'] = 'gain'

# Add correct choice column and rename 1 and 0 to yes and no.
d1$correct = c()
d1[d1$hit == 1, 'correct'] = 'yes'
d1[d1$hit == 0, 'correct'] = 'no'

d2$correct = c()
d2[d2$hit == 1, 'correct'] = 'yes'
d2[d2$hit == 0, 'correct'] = 'no'
...
brain_regions = colnames(df[24:68])
...
# calculate mean activity by (ses, hemisphere, condition, tr)
# piping the data into the summarize mean activity function by every combination of those experimental variables.

result = df.long %>%
  group_by(subject, region, value, ses, rewardcond, hemi,
           tr) %>% summarise(mean_activity = mean(value))

# Will need to create more data frames that also test bp (button press R or L), and hits (correct=1, incorrect=0). Will have to seperate into more dataframes to test mean activity based on more combinations that take choice into consideration. 
...
dfinal = dcast(result, subject ~ region + ses + rewardcond + hemi + tr,
                fun.aggregate = mean,
...
# region_session_condition_hemisphere_tr
require(psych)
...
ICC(dfinal[, c("nacc_mni_s1_gain_b_5", "nacc_mni_s2_gain_b_5")])
ICC(dfinal[, c("nacc_mni_s1_gain_l_3", "nacc_mni_s2_gain_l_3")])
ICC(dfinal[, c("nacc_mni_s1_gain_l_4", "nacc_mni_s2_gain_l_4")])
ICC(dfinal[, c("nacc_mni_s1_gain_l_5", "nacc_mni_s2_gain_l_5")])
ICC(dfinal[, c("nacc_mni_s1_gain_l_6", "nacc_mni_s2_gain_l_6")])

ICC(dfinal[, c("nacc_mni_s1_gain_r_3", "nacc_mni_s2_gain_r_3")])
ICC(dfinal[, c("nacc_mni_s1_gain_r_4", "nacc_mni_s2_gain_r_4")])
ICC(dfinal[, c("nacc_mni_s1_gain_r_5", "nacc_mni_s2_gain_r_5")])
ICC(dfinal[, c("nacc_mni_s1_gain_r_6", "nacc_mni_s2_gain_r_6")])
...
ICC(dfinal[, c("nacc_mni_s1_neutral_b_5", "nacc_mni_s2_neutral_b_5")])
...
ICC(dfinal[, c("ains_s1_gain_r_5", "ains_s2_gain_r_5")])
...
ICC(dfinal[, c("ains_mni_s1_gain_r_5", "ains_mni_s2_gain_r_5")])
...
ICC(dfinal[, c("ains_s1_loss_r_5", "ains_s2_loss_r_5")])
ICC(dfinal[, c("ains_s1_loss_r_6", "ains_s2_loss_r_6")])
...
ICC(dfinal[, c("ains_mni_s1_loss_r_5", "ains_mni_s2_loss_r_5")])
...
ICC(dfinal[, c("ains_kg_s1_loss_r_5", "ains_kg_s2_loss_r_5")])
